...
1. 데이터 전처리 : 데이터 생성, 정제, 변환, 결합
    ...
    2) 이상치 판별
        (1) 박스플롯을 통해 Q1-1.5*IQR과 Q3+1.5*IQR의 범위를 넘어가는 자료를 이상값으로 진단
        (2) 표준화 점수(Z-score)의 절대값이 2, 3보다 큰 경우 이상값으로 진단
    3) 이상값 처리
        (1) 이상값 제외
        (2) 이상값 대체 : 이상값을 정상 범위의 최소값, 최대값으로 대체
        (3) 변수 변환 : 자료값 전체에 로그변환, 제곱근 변환 등 적용
    4) 연속형 자료 범주화
        (1) 방법 : 연속형 변수를 구간으로 나누어 범주화하여 이상치 문제 완화
        (2) 효과 : 이상치 문제 완화, 결측치 처리 방법, 과적합 방지 및 결과 해석 용이
    5) 데이터 변환
        (1) 목적 : 분포의 대칭화(정규화), 그룹간 비슷한 산포 유지, 변수간 관계 단순화(비선형→선형) 목적
        (2) 유형
...
2. 머신러닝
    ...
    4) 분석 절차
        ...
        (2) 적절한 모델 선택
        (3) 데이터로 모델 학습
        (4) 훈련된 모델로 새로운 데이터 예측

    5) 검증 및 평가 : 과대적합 방지 및 일반화 오차 최소화 방법
        (1) Hold-out 방식 : 데이터를 Training 60% (Validation 20%,) Test 20%로 분리 (비율은 조정 가능)
            - 검증 데이터(Validation data) : 하이퍼파라미터 조정, 변수 선택, 모델 튜닝에 사용
            - 평가 지표로 train, test 데이터 비교 후 차이를 통해 모델 적합성 판단
        (2) k-fold 교차검증(Cross-validation) : 데이터가 적을 때, k개로 분할하여 train(k-1), test(1) 번갈아가며 전체 반복, 예측오차의 평균 계산
...
3. 머신러닝 모델의 평가지표
    1) 회귀 모델
        (1) RMSE (Root Mean Square Error) : 오차 제곱의 평균에 제곱근을 씌운 값
        (2) R-square(결정계수) : 0은 오차가 크고, 1은 오차가 거의 없음
        (3) MAE(Mean Absolute Error) : 오차 절대값의 평균
        (4) MAPE(Mean Average Percentage Error) : 실제 값 대비 오차 비율의 평균

    2) 분류 모델
        (0) 정오분류표(confusion matrix)
        (1) 정확도(Accuracy) : 맞춘 경우 / 전체 경우
...
4. 특성 공학
    ...
    1) 특성 선택 : 주어진 특성 변수 중 가장 좋은 변수만 선택
        (1) filter : 각 특성과 목표변수의 연관성(1:1) 평가 (t-test, chi-square, information gain 등)
        (2) wrapper : 다양한 변수 조합으로 모델 학습·평가 (forward, backward, stepwise selection 등)
        (3) embedded : 알고리즘 내 직접 변수 최적화 (Ridge, Lasso, Elastic net 등)

    2) 특성 추출(차원축소법, 비지도학습)
        (1) 주성분 분석(PCA) : 변수들에 담긴 정보를 최대한 확보해 새로운 변수(주성분) 생성, 변동이 큰 축 탐색, 손실 최소화, 상관 없는 주성분으로 해석 용이
        (2) 특성값 분해(SVD) : 정보량이 많은 순서대로 m개만 이용해 데이터 근사
...
5. 비지도학습 - 군집분석 (차원축소법도 비지도학습)
    ...
    2) 계층적 군집분석 : 병합적(가까운 것 순차적 묶기), 분할적(먼 것 나누기)
        ...
        (2) 덴드로그램 : 개체별 클러스터링 과정을 시각화한 그림

    3) 비계층적 군집분석
        (1) K-means 군집분석 : 사전 결정된 k만큼 군집 형성, 계산량 적고 대용량 데이터에 빠름. 초기 군집 중심 위치, 이상치 영향에 민감
        (2) k-means 알고리즘 : k개로 초기 군집화 → 군집 중심 계산 후 가장 가까운 군집에 할당 → 중심 재계산 → 반복
        (3) 군집 수 결정 : 군집수 k별 SSE(오차제곱합) 변화 관찰, SSE가 급격히 감소하다 완만해지는 지점의 k 선정

6. 지도학습 - 회귀분석 : 독립변수-종속변수 간 함수적 관련성을 밝히기 위해 수학적 모형을 세워, 이를 통계적으로 추정하는 분석 방법
    1) 단순선형회귀분석
        (1) 정의 : 독립변수 1개, 오차항은 독립적인 정규·등분산·독립 가정의 확률변수
        (2) 모수 의미 : α(알파), β(베타) 회귀계수, ε(엡실론) 오차항
        (3) 모수 추정(최소제곱법) : 수직거리 제곱합 SS(α, β)가 최소가 되도록 추정
        (4) 모수에 관한 가설검정(t 검정)
            - 귀무가설 H0 : β=0 (X와 Y 무관)
            - 대립가설 H1 : β≠0 (X와 Y 관련)
    ...
    2) 다중선형회귀분석
        (1) 정의 : 독립변수 2개 이상
        (2) 모수 의미 : α, β₁... 회귀계수, ε 오차항
        (3) 모수 추정법 : 최소제곱법
        (4) 적합도(R-square) : 단순회귀와 동일, 범주형 독립변수는 더미변수 사용
        (5) 변수선택(wrapper) : forward, backward, stepwise
        ...
        (7) 다중공선성 : 독립변수 간 강한 상관관계로 회귀계수 추정에 부정적 영향

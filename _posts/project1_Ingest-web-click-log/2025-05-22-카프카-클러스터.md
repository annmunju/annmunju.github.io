...
이제 본격적으로 **카프카 클러스터**를 구성하고 로그 데이터를 수신할 수 있도록 세팅해보려고 합니다.  
여기서 제가 가장 중요하게 생각한 건 **비용**입니다.  
카프카의 구성을 직접 익혀보고 어떤 방식으로 쓰는지 알기 위한 PoC이기 때문에,  
설치와 운영에 드는 비용을 최대한 줄이고자 **EKS, MSK 같은 매니지드 서비스를 쓰지 않고 직접 EC2 인스턴스에 카프카를 띄워서 사용**하기로 했습니다!
...
이번 글에서는 **로컬 Docker Compose**, **EC2 기반 설치**, **systemd 서비스 등록**을 통해 최소 비용으로 Kafka 단일 브로커 환경을 띄워보고, 토픽 생성과 프로듀서·컨슈머 테스트까지 완료했습니다. 직접 설치·운영 과정을 경험하며 Kafka의 내부 동작 원리를 이해하고, 부트스트랩·리스너 설정, Zookeeper 연동, systemd를 이용한 자동 기동 방법을 익혔습니다.

앞으로는 이 PoC를 확장하여 **데이터 파이프라인**을 완성하는 단계로 나아갑니다.
다음 글에서는 **수집 로그를 구체화 하는** 과정을 공유드리겠습니다.

...
이제 본격적으로 카프카 클러스터를 구성하고 로그 데이터를 수신할 수 있도록 세팅해보려고 합니다. 여기서 제가 가장 중요하게 생각한 건 **비용**입니다. 카프카의 구성을 직접 익혀보고 어떤 방식으로 쓰는지 알기 위한 PoC이기 때문에, 설치와 운영에 드는 비용을 최대한 줄이고자 EKS, MSK 같은 매니지드 서비스를 쓰지 않고 직접 EC2 인스턴스에 카프카를 띄워서 사용하기로 했습니다!
...
카프카 클러스터 구축을 위해서 도커 컴포즈 파일 작성 후 실행합니다.
```yml
# docker-compose.yml
version: '3.8'
services:
...
```
- **zookeeper?**
    - 아파치에서 만든 **분산 코디네이션 서비스**입니다.
    - 카프카 클러스터에서 브로커 간의 **메타데이터 관리, 리더 선출, 구성원 변화 감지** 등을 담당해요. (카프카는 메시지 저장을 담당하고, 어떤 파티션이 몇 개며 어디에 위치하는지 같은 정보는 주키퍼에 보관)
- 카프카 브로커가 시작하면서 "내가 클러스터의 일원"임을 주키퍼에 등록하고 기존 브로커들과 토픽, 파티션 메타데이터를 주고 받습니다.
- 리스너는 PLAINTEXT로 암호화나 인증 없이 연결합니다.

`docker-compose up -d` 실행시 백그라운드에 띄워집니다.
...
```
- 단순 테스트용으로 파티션과 복제는 하나만 생성합니다.

#### 1.3 Producer 코드 작성
다음과 같은 fastapi 코드를 작성해 producer 역할을 하도록 합니다.
```python
# ingest_kafka.py
...
```
이제 이 코드를 `uvicorn ingest_kafka:app --host 0.0.0.0 --port 8000`로 로컬에서 실행시켜두었습니다. 해당 api로 post 요청을 보내면 메시지가 생성됩니다.

#### 1.4 Consumer 실행
이제 카프카의 컨슈머를 실행해 로그를 살펴봅니다.
```bash
docker exec -it clickstream-test-kafka-1 \
...
```
해당 코드로 컨슈머를 실행시킨 상태에서, 아까 띄워둔 producer 역할을 하는 api에 curl 명령을 보내 메시지를 생성합니다.
```bash
# 예시
...
```
그럼 카프카의 컨슈머에 해당 메시지가 전달되어 출력됩니다.
...
이제 로컬 테스트를 마쳤으니, Terraform으로 프로비저닝한 EC2 인스턴스 위에 Kafka 클러스터를 배포하고 외부에서 로그를 수신하는 과정을 진행합니다.

#### 2.1 EC2 인프라 코드 (Terraform)
EC2 인스턴스를 Terraform으로 프로비저닝하고, User Data를 통해 Zookeeper와 Kafka를 자동으로 설치하고 이를 AMI로 만들어 둡니다.
테라폼은 사전 설치 되어있고 aws 접근을 위한 권한을 갖추기 위해 테라폼 전용 사용자를 생성합니다.

> 처음부터 최소 권한을 준수하면서 테라폼을 실행시키면 권한 문제 에러가 자주 발생합니다. [일단 전체 권한을 준 뒤에 CloudTrail 활동을 기반으로 선별된 정책으로 권한을 축소하는 방법도 있으니 참고해주세요.](https://docs.aws.amazon.com/IAM/latest/UserGuide/access-analyzer-policy-generation.html?utm_source=chatgpt.com#access-analyzer-policy-generation-api) 
{: .prompt-danger }
...
이제 생성한 엑세스 키를 개발 서버에 `aws configure` 적용하고 해당 서버에서 테라폼 코드를 실행시키게 됩니다.

- 기존에 있던 VPC, Subnet 사용. security group 만들기 (포트번호 22:ssh, 2181:zookeeper, 9092:kafka 인바운드 허용)
- EC2 생성시에 EBS 생성 (영구 볼륨)

> 기존 계획에는 프라이빗 서브넷에 설치하고자 했으나 ec2 내부에 원하는 패키지 설치가 어려워서 퍼블릭 서브넷에서 설치하고 초기 세팅을 마친 뒤 AMI로 만들어서 사용했습니다.
{: .prompt-tip}

#### 2.2 EC2에 카프카 설치 및 동작
카프카를 설치하기 위한 bash 스크립트입니다. 로컬에서 테스트한 버전과 동일하게 설정했고 EC2 AMI는 아마존 리눅스 2를 사용했습니다.
```bash
#!/bin/bash
# 1) 시스템 업데이트
yum update -y
...
이렇게 실행하게 되면 매번 수동으로 명령을 실행해야해 서비스로 등록하도록 아래 작업을 진행했습니다. systemd 서비스 파일을 생성하고 enable만 해놓으면 부팅시에 자동으로 시작됩니다.
...
/opt/confluent/bin/kafka-topics \
  ...
  --replication-factor 1
```
이렇게 토픽을 생성한 후 프로듀서에 메시지를 보내고, 컨슈머에 메시지를 잘 받아오는지 아래와 같이 확인했습니다.
```bash
# Producer
...
이번 글에서는 **로컬 Docker Compose**, **EC2 기반 설치**, **systemd 서비스 등록**을 통해 최소 비용으로 Kafka 단일 브로커 환경을 안정적으로 띄워보고, 토픽 생성과 프로듀서·컨슈머 테스트까지 완료했습니다. 직접 설치·운영 과정을 경험하며 Kafka의 내부 동작 원리를 이해하고, 부트스트랩·리스너 설정, Zookeeper 연동, systemd를 이용한 자동 기동 방법을 익혔습니다.

앞으로는 이 PoC를 확장하여 **데이터 파이프라인**을 완성하는 단계로 나아갑니다:

1. **Lambda → Kafka 연동**  
   - 앞서 만든 Ingest API를 AWS Lambda로 이전하고, VPC 설정을 통해 Kafka 브로커로 직접 프로듀싱  
2. **배치 저장소 구성**  
   - Kafka Connector 또는 커스텀 스크립트를 이용해 S3·HDFS에 주기적 데이터 적재  
3. **분석 환경 구축**  
   - Spark 또는 Flink 클러스터를 연동해 실시간·배치 분석 워크플로우 구현  
4. **실시간 시각화**  
   - Grafana/QuickSight와 연동하여 클릭스트림 대시보드 제작  
5. **확장 & 운영 안정화**  
   - 다중 브로커·파티션 분할을 통한 가용성 개선  
   - Prometheus·CloudWatch 기반 모니터링·알람 설정  

다음 글에서는 **AWS Lambda를 활용한 Kafka 프로듀싱**, **S3 배치 적재** 과정을 중심으로 단계별 코드 예제와 Terraform 설정을 공유드리겠습니다. 

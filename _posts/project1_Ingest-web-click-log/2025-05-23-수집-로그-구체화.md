...
## 1. 확정된 주제 소개
블로그를 운영하다 보면, 어떤 외부 채널에서 어떤 게시글로 방문이 몰리는지 파악하는 것이 중요합니다.  
이 글에서는 **어떤 경로에서 어떤 게시글이 가장 많은 관심을 받았는지**를 명확하게 파악하기 위한 로그를 정의하고자 합니다.
...
이 글에서는 바로 이 지점을 짚어, 분석하고자 하는 목표를 명확히 정의한 뒤 필요한 로그만 골라 수집하는 과정을 보여드리고자 합니다.

그래서 수집된 로그를 활용하는 내용까지 구체적으로 보여줄 수 있는 확장 가능한 프로젝트가 될 수 있을 거에요!

> 💡 **핵심 주제**  
> **블로그 유입 경로별 인기 게시글 분석**  
> 외부 채널(referrer)별로 어떤 게시글이 얼마나 많은 방문을 기록했는지 파악하고,  
> 이를 통해 콘텐츠 배포 전략을 설계해봅니다.
...
## 3. 프론트엔드 로그 수집 코드 구현
자, 이제 위와 같이 적용한 로그 목록에 맞게 프론트엔드에 삽입할 자바스크립트 코드를 다음과 같이 작성했습니다. (with GPT🤖)
...
이번 글에서는 "**블로그 유입 경로별 인기 게시글 분석**"을 위한 클릭스트림 로그 수집 파이프라인을 완성했습니다.  
